...
const handleUpdateQunaity=async(product)=>{
  ...
  return (
<>
<GeneralPageCompenentAdmin title={"Hello Admin Here you can Manage your Quaintity"}  />
        ...
        {productsall.map((product, index) => (
          <ListItem key={index}>
            <ListItemText
              primary={
                <>
<Box sx={{ display: "flex", flexDirection: "row", alignItems: "center",backgroundColor:"wheat" }}>
  <Typography variant="h5" color="initial">
   {index+1}. Title: {product?.title}{" "} {" "} Quantity: {product?.quantity}
  </Typography>
  <Box sx={{ ml: "auto" }}>
    <IconButton disabled={product?.quantity > 100} onClick={()=>handleUpdateQunaity(product)}>
      <Typography
        variant="body1"
        color="initial"
        p={1}
        sx={{ color: product?.quantity > 900 ? "green" : "red" }}
      >
    Purchase Quantity
      </Typography>
      <UpdateIcon />
